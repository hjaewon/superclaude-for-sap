...
원본 sc4sap 플러그인(MIT)을 **내 GitHub 포크로 떠서**, **"ABAP 공식문서를 실제로 읽어오는 패치"**를 넣고, 여러 컴퓨터에서 내 포크로 설치·관리하려는 작업. **패치는 로컬에 완성됐고, 아직 커밋·푸시·설치 전.**
...
5. **마켓플레이스 이름 충돌**: 내 포크의 `marketplace.json` 이름도 `sc4sap`라, 기존 github판이 있는 PC에선 **기존 것을 먼저 제거**해야 함.
...
### STEP 1 — 커밋 & 포크에 푸시  (이 클론에서)
```bash
cd "D:\Claude for SAP\supersap_custom"
git add -A
git commit -m "feat(doc-specialist): browserless ABAP keyword doc fetcher (.html); bump 0.6.15"
git push origin main
```
- 첫 푸시 때 GitHub 인증 필요(PAT/자격증명 관리자).
- ⚠️ `git add -A`는 이 `WORK_LOG.md`도 같이 스테이징함. 포크에 올리기 싫으면 먼저 `.gitignore`에 추가하거나 개별 `git add` 사용.
...
2. `plugin.json` + `marketplace.json` **version 올리기** (예: 0.6.15 → 0.6.16) — 업데이트 인식 트리거
3. `git add -A && git commit -m "..." && git push origin main`
4. 각 PC의 Claude Code에서: `/plugin marketplace update sc4sap` → `/plugin install sc4sap@sc4sap` → 재시작/reload
